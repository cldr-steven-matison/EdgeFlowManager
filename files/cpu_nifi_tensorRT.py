...
# CPU-only variant of the TensorRT enrichment processor. The KubernetesPod agent
# has no GPU, so we skip the `tensorrt` import and stamp the payload with CPU
# metadata instead — schema stays the same shape so downstream flows don't care.
def onTrigger(context, session):
    ...
    if flow_file:
        try:
            ...
            # 2. CPU inference metadata (mirrors the GPU version's shape)
            cpu_info = {
            ...
            # 3. Append to JSON structure cleanly. Key is `tensorrt` so the
            #    downstream contract matches the GPU variant.
            if isinstance(payload, dict):
                payload['tensorrt'] = cpu_info
            elif isinstance(payload, list):
                for item in payload:
                    if isinstance(item, dict):
                        item['tensorrt'] = cpu_info
            ...
            session.putAttribute(flow_file, "python.tensorrt.backend", "cpu")

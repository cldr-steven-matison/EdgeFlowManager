...
This chapter runs plain Apache MiNiFi Java in minikube on macOS with no EFM involvement at all, the Java counterpart to [Chapter 7](ch07-standalone-minifi-cpp-on-k8s.md)'s standalone C++ agent. The flow is `ListenHTTP (8080)` → `PutFile` (`/tmp/minifi-test-output`), with the flow baked into the image as a `config.yml`. Everything here comes from the [MiNiFi Kubernetes Playground](https://github.com/cldr-steven-matison/MiNiFi-Kubernetes-Playground) repo, the scripts and the YAML as they run. EFM does not enter the picture until [Chapter 9](ch09-efm-in-the-playground.md).

## What This Scenario Shows

Standalone MiNiFi Java, with `config.yml` baked into the image, is the same fast iteration loop the C++ agent gives you. No EFM, no agent registration, no class reconciliation. You edit `config.yml`, rebuild, and the pod restarts with the new flow. It also establishes what changes when you move from C++ to Java before EFM is anywhere in the picture. The flow config format, the class names, the image footprint, and the way the pod reports ready. Getting those right standalone means they are already right when EFM takes over the same agent in Chapter 9.

The standalone image is `container.repo.cloudera.com/cloudera/nifi-minifi-java:latest`, Apache MiNiFi Java `1.23.04-b15`, a `config.yml`-driven runtime. `MINIFI_HOME=/opt/minifi/minifi-1.23.04-b15`, which `docker run --rm <image> find /opt -iname "*minifi*"` shows. (The EFM-managed CEM agent introduced later is a different, `2.24.08.0-19` runtime, Chapter 9 onward.)
...
`MINIFI_HOME=/opt/minifi/minifi-1.23.04-b15` comes from the image itself. Pull a different version of the image and that path changes. The version number is part of the directory name, and a wrong `MINIFI_HOME` means `config.yml` lands where the agent never reads it.
...
Two more numbers that differ from C++. Memory (`512Mi` request / `1Gi` limit, since the JVM will not run in the C++ agent's `128Mi`) and `initialDelaySeconds: 60` (the JVM plus MiNiFi bootstrap takes 30 to 60 seconds, and a 5-second delay loop-restarts the pod before the agent is up).

## Checking Delivery

**1. Open the network tunnel.** On macOS, minikube NodePorts are not reachable from `localhost` directly. Run this in a dedicated terminal and leave it open.

```bash
minikube service minifi-test-java-service --url
```

**2. POST a test message.** `ListenHTTP` on the Java agent takes `POST` only, on `/contentListener`.
...
**3. Check PutFile.** Look at the internal pod storage.

```bash
kubectl exec -it deployment/minifi-test-java -- /bin/sh -c "cat /tmp/minifi-test-output/*"
```

The JSON body you POSTed appears here. `ListenHTTP → PutFile` is wired from the `config.yml`.

## What NOT to Do

**Do not use an `httpGet` readiness probe against `/contentListener`.** Java's `ListenHTTP` returns `405` to a `GET`, so the probe never passes and the pod stays `NotReady` forever. Use a `tcpSocket` probe against port 8080.

**Do not give the Java pod the C++ agent's `128Mi`.** The JVM will not start. `512Mi` request / `1Gi` limit is the minimum.

**Do not set `initialDelaySeconds: 5` like the C++ deployment.** The JVM plus MiNiFi bootstrap takes 30 to 60 seconds. A 5-second delay loop-restarts the pod before it finishes coming up.

**Do not use C++ short class names in the Java `config.yml`.** `class: ListenHTTP` fails to instantiate. The Java runtime needs the full FQCN `org.apache.nifi.processors.standard.ListenHTTP`.

**Do not wire connections by name in a Config Version 3 file.** Java connections reference `source id`/`destination id` (the component UUIDs), not `source name`/`destination name`. Mixing the C++ style here produces a flow that loads with no connections.

**Do not skip `eval $(minikube docker-env)` before `docker build`.** The image lands on your Mac's daemon, the minikube node never sees it, and the pod goes `ErrImageNeverPull`. It does not persist across terminal sessions. Run it every build.

## Related Chapters

- [Standalone MiNiFi C++ on Kubernetes](ch07-standalone-minifi-cpp-on-k8s.md) (Ch7): the C++ agent this chapter mirrors, and the shared playground and rebuild loop.
- [Introduce EFM into the Playground](ch09-efm-in-the-playground.md) (Ch9): where EFM takes over managing these same agents.
- [MiNiFi Java Processor Catalog](ch04-java-processor-catalog.md) (Ch4): the full Java processor and controller-service set, and the Kafka/scripting NAR drop-in for the EFM-managed CEM agent.
